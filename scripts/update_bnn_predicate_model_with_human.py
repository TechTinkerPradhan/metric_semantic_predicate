...
from metric_semantic_predicate.models.bnn_predicate_model import (
    train_bnn_pred_model,
    bnn_predict_predicate,
    BayesianNNPred,
    guide_bnn_pred
)
...
def main():
    ...
    # Load and register pretrained model
   
    net = BayesianNNPred(
        input_dim=X_h_train.shape[1],
        hidden_dim=cfg['training']['hidden_dim'],
        output_dim=Y_h_train.shape[1]
    ).to(device)

    # 2. Load the saved parameters
    load_model(model_name)

    # 3. Register parameters from loaded model
    
    guide_bnn_pred(
        torch.zeros(1, X_h_train.shape[1]).to(device),
        torch.zeros(1, Y_h_train.shape[1]).to(device),
        net
    )

    # 4. Now you're safe to call prediction
    mean_old, _ = bnn_predict_predicate(X_h_test_t, net)

    # Fine-tune with human-labeled data
    ...
    load_model(model_name)
    guide_bnn_pred(torch.zeros(1, X_orig_test.shape[1]).to(device), torch.zeros(1, Y_orig_test.shape[1]).to(device), net_before)
